...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("OGL_Concentric_Circles"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
...
void display(void)
{
	...
	for (double angle = 0.0f; angle < 360; angle = angle + 0.01)
	{
		glVertex3f(cos(angle), sin(angle), 0.0f);
	}

	glColor3f(0.0f, 1.0f, 0.0f);
	for (double angle = 0.1f; angle < 360; angle = angle + 0.001)
	{
		glVertex3f(cos(angle)/0.9f, sin(angle)/0.9f, 0.0f);
	}

	glColor3f(0.0f, 0.0f, 1.0f);
	for (double angle = 0.1f; angle < 360; angle = angle + 0.001)
	{
		glVertex3f(cos(angle) / 1.1f, sin(angle) / 1.1f, 0.0f);
	}

	glColor3f(1.0f, 0.5f, 1.0f);
	for (double angle = 0.1f; angle < 360; angle = angle + 0.001)
	{
		glVertex3f(cos(angle) / 1.25f, sin(angle) / 1.25f, 0.0f);
	}
	...
	for (double angle = 0.1f; angle < 360; angle = angle + 0.001)
	{
		glVertex3f(cos(angle) / 1.85f, sin(angle) / 1.85f, 0.0f);
	}

	for (double angle = 0.1f; angle < 360; angle = angle + 0.001)
	{
		glVertex3f(cos(angle) / 2.2f, sin(angle) / 2.2f, 0.0f);
	}

	for (double angle = 0.1f; angle < 360; angle = angle + 0.001)
	{
		glVertex3f(cos(angle) / 2.7f, sin(angle) / 2.7f, 0.0f);
	}
	glEnd();
