...
DWORD WINAPI ThreadPlaySound(LPVOID);
HDC ghdc = NULL;
HWND ghwnd = NULL;
HGLRC ghrc = NULL;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_CREATE:
		hThreadPlaySound = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ThreadPlaySound, (LPVOID)hwnd, 0, NULL);
		ToggleFullScreen();
		break;
	case WM_SETFOCUS:
		gbActiveWindow = true;
...
DWORD WINAPI ThreadPlaySound(LPVOID lParam)
{
	//PlaySound(TEXT("Final-Working.wav"),NULL, SND_SYNC | SND_NODEFAULT);
	PlaySound(TEXT("WAV_PlaySound"), GetModuleHandle(NULL), SND_RESOURCE|SND_ASYNC);
	return 0;
}
...
void display(void)
{
	...
	static double ixAxis = -1.0f, axAxis = 2.0f, nyAxis = 2.0f, iyAxis = -2.0f, xAxisPlane = -5.8f, xAxisForColor = -1.0f, xAxisPlaneMiddle = -9.0f,
		rColor1 = 0.9f, gColor1 = 0.9f, bColor1 = 0.9f,
		rColor2 = 0.9f, gColor2 = 0.9f, bColor2 = 0.9f,
		rColor3 = 0.9f, gColor3 = 0.9f, bColor3 = 0.9f,
		yAxisPlaneBottom = -8.0f, xAxisPlaneBottom = -9.0f,
		xAxisPlaneUp = -9.0f, yAxisPlaneUp = 6.0f;
	static bool bColorFlag = true;
	glClear(GL_COLOR_BUFFER_BIT);
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();


	glTranslatef((GLfloat)ixAxis, 0.0f, 0.0f);
	...
	if (ixAxis >= 0.0f)
	{
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef((GLfloat)axAxis, 0.0f, 0.0f);
	...
	if (axAxis <= 0.2f && ixAxis >= 0.0f)
	{
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef(0.02f, (GLfloat)nyAxis, 0.0f);
	...
	if (axAxis <= 0.2f && ixAxis >= 0.0f && nyAxis <= 0.0f)
	{
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef(0.2f, (GLfloat)iyAxis, 0.0f);
	...
	//Plane Code
	if (axAxis <= 0.2f && ixAxis >= 0.0f && nyAxis <= 0.0f && iyAxis >= 0.0f && bColorFlag==true)
	{
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef((GLfloat)xAxisPlaneMiddle, 0.0f, 0.0f);
		...
		xAxisPlaneMiddle =(GLfloat)( xAxisPlaneMiddle + 0.0004f);
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef((GLfloat)xAxisPlaneMiddle, 0.0f, 0.0f);
		gluLookAt(0, 0, 8, 0, 0, 0, 0, 1, 0);
		...
		{
			glColor3f((GLfloat)rColor1, (GLfloat)gColor1, (GLfloat)bColor1);
			rColor1 =(GLfloat)( rColor1 - 0.001f);
			gColor1 = (GLfloat)(gColor1 - 0.001f);
			bColor1 = (GLfloat)(bColor1 - 0.001f);

		}
		glLineWidth(5.0f);
		glBegin(GL_LINES);
		glVertex2f((GLfloat)xAxisForColor, 0.0f);
		glVertex2f(0.0f, 0.0f);
		glEnd();
		xAxisForColor =(GLfloat)( xAxisForColor - 0.0001f);
		...
		if (xAxisPlaneMiddle >= 6.5f)
		{
			glColor3f((GLfloat)rColor2, (GLfloat)gColor2,(GLfloat) bColor2);
			rColor2 = (GLfloat)(rColor2 - 0.001f);
			gColor2 = (GLfloat)(gColor2 - 0.001f);
			bColor2 = (GLfloat)(bColor2 - 0.001f);

		}
		glLineWidth(5.0f);
		glBegin(GL_LINES);
		glVertex2f((GLfloat)xAxisForColor, -0.1f);
		glVertex2f(0.0f, -0.1f);
		glEnd();
		xAxisForColor = (GLfloat)(xAxisForColor - 0.001f);
		...
		if (xAxisPlaneMiddle >= 6.5f)
		{
			glColor3f((GLfloat)rColor3, (GLfloat)gColor3,(GLfloat) bColor3);
			rColor3 = (GLfloat)(rColor3 - 0.001f);
			gColor3 = (GLfloat)(gColor3 - 0.001f);
			bColor3 = (GLfloat)(bColor3 - 0.001f);
		}
		glLineWidth(5.0f);
		glBegin(GL_LINES);
		glVertex2f((GLfloat)xAxisForColor, -0.2f);
		glVertex2f(0.0f, -0.2f);
		glEnd();
		...
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef((GLfloat)xAxisPlaneBottom, (GLfloat)yAxisPlaneBottom, 0.0f);
		gluLookAt(0, 0, 8, 0, 0, 0, 0, 1, 0);
		glLineWidth(6.0f);
		glColor3f(176.0f / 255.0f, 224.0f / 255.0f, 230.0f / 255.0f);
		//2nd  plane
		...
		if (xAxisPlaneBottom >= 2.9f)
		{
			yAxisPlaneBottom = (GLfloat)(yAxisPlaneBottom + 0.0004f);
			xAxisPlaneBottom = (GLfloat)(xAxisPlaneBottom + 0.0004f);
		}
		else
		{
			xAxisPlaneBottom = (GLfloat)(xAxisPlaneBottom + 0.0004f);
		}
		if (yAxisPlaneBottom <= 0.015f)
			yAxisPlaneBottom = yAxisPlaneBottom + 0.001f;



		//1st Quads for 2nd plane
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef((GLfloat)xAxisPlaneBottom, (GLfloat)yAxisPlaneBottom, 0.0f);
		gluLookAt(0, 0, 8, 0, 0, 0, 0, 1, 0);
		...
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef((GLfloat)xAxisPlaneUp, (GLfloat)yAxisPlaneUp, 0.0f);
		gluLookAt(0, 0, 8, 0, 0, 0, 0, 1, 0);
		glLineWidth(6.0f);
		glColor3f(176.0f / 255.0f, 224.0f / 255.0f, 230.0f / 255.0f);
		//3rd  plane
		...
		if (xAxisPlaneUp >= 2.9f)
		{
			yAxisPlaneUp = (GLfloat)(yAxisPlaneUp - 0.0004f);
			xAxisPlaneUp = (GLfloat)(xAxisPlaneUp + 0.0004f);
		}
		else
		{
			xAxisPlaneUp = (GLfloat)(xAxisPlaneUp + 0.0004f);
		}
		if (yAxisPlaneUp >= 0.015f)
			yAxisPlaneUp = (GLfloat)(yAxisPlaneUp - 0.0007f);

		//1st Quads for 3rd plane
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef((GLfloat)xAxisPlaneUp, (GLfloat)yAxisPlaneUp, 0.0f);
		gluLookAt(0, 0, 8, 0, 0, 0, 0, 1, 0);
		...
		{
			glColor3f((GLfloat)rColor3, (GLfloat)gColor3, (GLfloat)bColor3);
			rColor3 = (GLfloat)(rColor3 - 0.0001f);
			gColor3 = (GLfloat)(gColor3 - 0.0001f);
			bColor3 = (GLfloat)(bColor3 - 0.0001f);
		}
		glLineWidth(5.0f);
		glBegin(GL_LINES);
		glVertex2f((GLfloat)xAxisForColor, -0.2f);
	...
	if (xAxisPlaneMiddle >= 6.5f)
	{
		...
		glLineWidth(6.0f);
		glBegin(GL_LINES);
		glColor3f(255.0f / 255.0f, 153.0f / 255.0f, 51.0f / 255.0f);//Safforn Color
		glVertex2f(1.331f, 0.0f);
		glColor3f(255.0f / 255.0f, 153.0f / 255.0f, 7.0f / 255.0f);//Safforn Color
		glVertex2f(1.465f, 0.0f);
		glEnd();
		...
		glLineWidth(6.0f);
		glBegin(GL_LINES);
		glColor3f(255.0f / 255.0f, 255.0f / 255.0f, 255.0f / 255.0f);//White Color
		glVertex2f(1.317f, -0.07f);
		glColor3f(255.0f / 255.0f, 255.0f / 255.0f, 255.0f / 255.0f);//White Color
		glVertex2f(1.484f, -0.07f);
		glEnd();
		...
		glBegin(GL_LINES);
		glBegin(GL_LINES);
		glColor3f(19.0f / 255.0f, 136.0f / 255.0f, 8.0f / 255.0f);//Green Color
		glVertex2f(1.302f, -0.14f);
		glColor3f(19.0f / 255.0f, 136.0f / 255.0f, 8.0f / 255.0f);//Green Color
		glVertex2f(1.499f, -0.14f);
		glEnd();
	}
	////IAF Letter
	//glMatrixMode(GL_MODELVIEW);
