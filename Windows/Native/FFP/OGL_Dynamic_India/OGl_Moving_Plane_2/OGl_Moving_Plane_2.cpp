...
void display(void)
{
	...
	static GLfloat xAxisPlane = -9.0f, xAxisPlaneMiddle = -9.0f,xAxisForColor = -1.0f,yAxisPlane=-9.0f, rColor1 = 0.9f, gColor1 = 0.9f, bColor1 = 0.9f,
	...
	xAxisPlaneMiddle = xAxisPlaneMiddle + 0.0004f;
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	glTranslatef(xAxisPlaneMiddle, 0.0f, 0.0f);
	gluLookAt(0, 0, 8, 0, 0, 0, 0, 1, 0);
	...
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	glTranslatef(xAxisPlane, yAxisPlane, 0.0f);
	gluLookAt(0, 0, 8, 0, 0, 0, 0, 1, 0);
	glLineWidth(6.0f);
	glColor3f(176.0f / 255.0f, 224.0f / 255.0f, 230.0f / 255.0f);
	//2nd  plane
	...
	if (xAxisPlane >= 2.9f)
	{
		yAxisPlane = yAxisPlane + 0.0004f;
		xAxisPlane = xAxisPlane + 0.0004f;
	}
	else
	{
		xAxisPlane = xAxisPlane + 0.0004f;
	}
	if(yAxisPlane<=0.015f)
	yAxisPlane = yAxisPlane + 0.001f;
	...
	//xAxisPlane2 = xAxisPlane2 + 0.001f;
	SwapBuffers(ghdc);
}
