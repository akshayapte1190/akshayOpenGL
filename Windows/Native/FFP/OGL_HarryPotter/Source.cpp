...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (fopen_s(&gpFile, "OGL_Concentric_Triangle.txt", "w") != 0)
	{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("OGL_Concentric_Triangle"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
...
void display(void)
{
	static GLfloat angle,xaxis=-1.0f,yaxis=-1.0f, circleYaxis=2.0f,yAxisangle;
	glClear(GL_COLOR_BUFFER_BIT);
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	glTranslatef(xaxis, yaxis, -1.5f);
	...
	if (xaxis >= 0)
	{
		glTranslatef(1.6f, circleYaxis, -15.0f);
		...
		for (int i = 0; i < NUM_POINTS; i++)
		{
			...
			angle = (GLfloat)(2.0*M_PI*i) / NUM_POINTS;
			glVertex3f((GLfloat)cos(angle), (GLfloat)sin(angle), 0.0f);

		}
		if (circleYaxis >= 0.0f)
		{
			circleYaxis = circleYaxis - 0.0001f;
		}
		yAxisangle = yAxisangle + 0.1f;
