...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("OGL_DoubleBuffer_Triangle_Perspective"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
...
void reshape(int width, int height)
{
	...
	{
		glViewport(width/2, 0, (GLsizei)width, (GLsizei)height);
	}
	else if (key == 4)
	{
		glViewport(0, height/2, (GLsizei)width, (GLsizei)height);
	}
	else if (key == 5)
	{
		glViewport(width/2, height/2, (GLsizei)width, (GLsizei)height);
	}
	else if (key == 6)
	{
		glViewport(width/2, 0, (GLsizei)width, (GLsizei)height/2);
	}
	else if (key == 7)
	{
		glViewport(0, height/2, (GLsizei)width/2, (GLsizei)height);
	}
	else if (key == 8)
	{
		glViewport(width/2, height/2, (GLsizei)width, (GLsizei)height/2);
	}
	else if (key == 9)
	{
		glViewport(width/2, height/2, (GLsizei)width/2, (GLsizei)height);
	}
